...
public class Sheep extends Herbivore {
    ...
    private double eat;
    public Sheep() {
        setMaxWeigth(Settings.MAX_WEIGHT_SHEEP);
        setMaxCapacity(Settings.MAX_CAPACITY_IN_ONE_CELL_SHEEP);
        setMaxFoodNeeded(Settings.MAX_FOOD_NEEDED_SHEEP);
    ...
    @Override
    public void move() {
        this.moverRandom = RandomizeClass.getRandom(0, Settings.MAX_SPEED_SHEEP);
        this.eat = RandomizeClass.getRandom();

        if (eat < 0.25) {
            chooseDirectionAhead(this.getX());
            setMaxWeigth(getMaxWeigth() - Settings.MINUS_HEALTH_ONE_STEP_SHEEP);
            if (getMaxWeigth() <= 0) {
                die(this);
            }
    ...
    public void die(Object death) {
        if (death instanceof Sheep && getMaxWeigth() <= 0) {
            atomicInteger.getAndDecrement();
            System.out.println("Овца, находящаяся в координатах: х - " + getX() + ", y - " + getY() + "умерла от голода :(((");
        }
    }
